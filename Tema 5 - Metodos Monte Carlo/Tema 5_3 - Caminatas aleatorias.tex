...
Las \textocolor{red}{caminatas aleatorias} son trayectorias en donde se dan pasos sucesivos en direcciones aleatorias.
\end{frame}
\begin{frame}
\frametitle{Definición}
...
% \begin{frame}
% \frametitle{Lanzamiento de monedas}
% ¿Qué tan grande será la suma luego de $N$ lanzamientos?
% \begin{align*}
% s_{N} = \nsum_{i = 1}^{N} \; \ell_{i} = \text{ águilas - soles}
% \end{align*}
% \end{frame}
% \begin{frame}
% \frametitle{Lanzamiento de monedas}
...
% \begin{frame}
% \frametitle{Lanzamiento de monedas}
% Podemos medir el valor absoluto promedio $\left\langle \abs{s_{N}} \right\rangle$, pero resulta que una distancia característica más agradable es la raíz media cuadrática (RMS) de la suma:
% \pause
% \begin{align*}
...
\bigskip
\pause
Es más conveniente estudiar los pasos a intervalos de tiempo regulares, por lo que consideraremos el \textocolor{cobalt}{problema clásico del borracho}.
\end{frame}
...
\bigskip
\pause
Es una interfaz gráficas popular para la acercar a los menores y adolescentes en la programación con \textocolor{blue}{\texttt{python}}.
\end{frame}
\begin{frame}
...
\bigskip
\pause
Cuando decimos \textocolor{blue}{azul}, \textocolor{red}{rojo} o \textocolor{ao(english)}{verde}, se habla de matices.
\end{frame}
\begin{frame}
...
\begin{frame}
\frametitle{El valor}
Por ejemplo, el \textocolor{coquelicot}{naranja} tiene un valor más alto que el \textocolor{blue(pigment)}{azul marino}. 
\\
\bigskip
\pause
