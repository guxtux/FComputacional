...
\textocolor{red}{La desventaja de este método es que aparecen extremos fijos}: \pause uno de los extremos de la sucesión de intervalos no se mueve del punto original, por lo que las aproximaciones a la raíz, denotadas por $b_{1}$, $b_{2}$, $b_{3}$, etc. convergen a la raíz exacta solamente por un lado.
\end{frame}
\begin{frame}
...
Dentro de la librería \funcionazul{numpy} tenemos disponible la función \funcionazul{roots} para calcular la(s) raíz(ces) de \textocolor{armygreen}{un polinomio}.
\end{frame}
\begin{frame}[fragile]
