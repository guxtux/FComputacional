...
Las \textocolor{cadmiumgreen}{listas} son el tipo de dato más versátil de los datos compuestos de \python.
\\
\bigskip
...
\\
\bigskip
Un punto importante de las listas: \pause es que \textocolor{ao}{todos los elementos pertenecientes a una lista pueden ser de tipo de datos diferente}.
\end{frame}
\begin{frame}[fragile]
...
\begin{frame}
\frametitle{Las listas en \python}
Los valores almacenados en una lista se recuperan usando la misma técnica de \textocolor{red}{slicing} que vimos con las cadenas: \pause $[ \: ]$ y $[:]$, \pause donde los índices van desde  $0$ hasta el   $n - 1$.
\end{frame}
\begin{frame}[fragile]
...
También se ocupan las operaciones de \textocolor{byzantine}{concatenación} ($+$) \pause y repetición en las listas ($*$):
\pause
...
Las listas en \python{} son los únicos objetos en los que podemos agregar nuevos elementos (son \textocolor{carmine}{mutables}), para ello hay que utilizar la función \textocolor{blue}{append}:
\pause
...
Una \textocolor{cobalt}{tupla} es otro tipo de datos de secuencia que es similar a la lista.
\\
\bigskip
\pause
...
Los \textocolor{blue-violet}{diccionarios} de \python{} son de tipo tabla-hash.
\\
\bigskip
\pause
Funcionan como matrices asociativas y consisten en pares \textocolor{cadmiumred}{llave} - \textocolor{cadmiumgreen}{valor}.
\end{frame}
\begin{frame}
\frametitle{Elementos del diccinario}
La \textocolor{cadmiumred}{llave} del diccionario puede ser casi de cualquier tipo de dato, pero suelen ser comúnmente números o cadenas.
\\
\bigskip
\pause
El \textocolor{cadmiumgreen}{valor}, por otra parte, puede ser cualquier tipo de objeto arbitrario de \python.
\end{frame}
\begin{frame}[fragile]
...
Los identificadores son nombres que hacen referencia a los objetos que componen un programa: \textocolor{burgundy}{constantes}, \textocolor{bulgarianrose}{variables}, \textocolor{burntorange}{funciones}, \textocolor{cadmiumred}{módulos}, \textocolor{cardinal}{clases} etc.
\end{frame}
\begin{frame}
\frametitle{Reglas para los identificadores}
...
from & lambda & return & break & system \\ \hline
...
Una estructura condicional inicialmente evalúa si una o más condiciones cumplen con un valor \textocolor{blue}{True}.
\\
\bigskip
\pause
...
\begin{frame}
\frametitle{Los condicionales}    
En caso de que el valor de la condición (o condiciones) no se cumpla, \pause es decir, tienen un valor \textocolor{red}{False}, \pause no se pasa al bloque con las instrucciones contenidas.
\\
\bigskip
\pause
...
El condicional \funcionazul{if} requiere de una expresión inicial que va a evaluar, como ya se mencionó, en caso de que no se cumpla el valor \textocolor{blue}{True}, no se ejecutan las instrucciones contenidas.
\begin{verbatim}
if expresion:
    instruccion1
    instruccion2
...
Cuando tenemos un condicional con \funcionazul{if} y la expresión que se evalúa tiene un valor \textocolor{red}{False}, sabemos que saldrá del bloque condicional.
\\
\bigskip
\pause
Pero si queremos que se ejecute otro bloque de instrucciones a pesar de que al inicio la expresión evaluada sea \textocolor{red}{False}, \pause recurrimos a la instrucción \funcionazul{else:}, por lo que se ejecutan las instrucciones contenidas dentro de este bloque.
\end{frame}
\begin{frame}[fragile]
...
\bigskip
\pause
Si \texttt{expresion2} devuelve un valor \textocolor{blue}{True}, entonces se ejecutan las instrucciones contenidas en ese bloque.
\end{frame}
\begin{frame}[fragile]
...
\bigskip
\pause
Todo aquel valor que se introduce, se toma como una \textocolor{carnelian}{cadena}, por ello se utiliza la función \funcionazul{int ()} para convertirla a un tipo de dato entero.
\end{frame}
...
\\
\bigskip
Se evalúa inicialmente una condición, en caso de que su valor sea (valor \funcionazul{True}) se ejecutan las instrucciones contenidas, \pause la repetición termina al cambiar el valor de la condición.
\end{frame}
\begin{frame}
...
\begin{enumerate}[<+->]
\conti    
\item Si le damos dos argumentos: \funcionazul{range(inicio, fin)}, generará una lista desde \textoazul{inicio} hasta \textoazul{fin - 1}.
\pause
\begin{align*}
\mbox{range}(1, 10)
\end{align*}
...
\begin{enumerate}[<+->]
\conti
\item Si le damos tres argumentos: \break \hfill \funcionazul{range(inicio, fin, paso)}, usará el \textoazul{paso} como incremento para generar los elementos de la lista.
\pause
\begin{align*}
\mbox{range}(1, 20, 3)
\end{align*}    
